...
possible_data_fillers = "./data_fillers.txt"
...
def write_event_filler(event_fillers, data_filler_names):
    with open(event_filler_location, "w") as event_filler_file:
        ...
        for event_filler in event_fillers:
            ...
            event_filler_file.write("    local event_data = {}\n\n")
            for event_data in event_filler["data"]:
                wrapcount = 0
                if event_data["optional"]:
                    event_filler_file.write(f'{" " * (4 + wrapcount * 4)}if event["{event_data["name"]}"] ~= nil then\n')
                    wrapcount += 1
                prefix = f'{" " * (4 + wrapcount * 4)}event_data["{event_data["name"]}"] = '
                if is_basic(event_data["type"]):
                    event_filler_file.write(f'{prefix}event["{event_data["name"]}"]\n')
                    if event_data["name"].endswith("_index") and event_data["name"][:-6] in data_filler_names:
                        name = event_data["name"][:-6]
                        event_filler_file.write(f'{" " * (4 + wrapcount * 4)}local {name} = game.get_{name}(event["{event_data["name"]}"])\n')
                        event_filler_file.write(f'{" " * (4 + wrapcount * 4)}if {name} ~= nil then\n')
                        wrapcount += 1
                        event_filler_file.write(f'{" " * (4 + wrapcount * 4)}event_data["{name}"] = ')
                        event_filler_file.write(f'data_fillers["{name}"]({name})\n')
